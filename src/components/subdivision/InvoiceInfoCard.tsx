...
import { ChevronUp, ChevronDown, LockOpen } from "lucide-react";
  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-card p-4 sm:p-6 relative">
      ...
      <div className="absolute -top-2 -right-2 sm:top-4 sm:right-4 flex items-center gap-2 z-40">
        <LockOpen size={22} className="sm:w-7 sm:h-7" style={{ color: "#757575" }} />
        {isLoaded && <span className="text-sm sm:text-2xl md:text-3xl text-secondary">2 d</span>}
      </div>
        ...
        <div className="flex flex-col justify-between space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4 pt-2">
            ...
            <Button
              ...
              disabled={!isLoaded}
              className="rounded-full px-4 sm:px-6 text-xs sm:text-sm disabled:opacity-100"
              style={{ backgroundColor: isLoaded ? "#79145C" : "#757575" }}
            >
              Subdividir
            </Button>
          </div>
